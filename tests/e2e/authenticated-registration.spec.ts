...
function requireRegistrationSecrets() {
  test.skip(
    ...
    'Authenticated registration E2E requires MELA_E2E_EMAIL, MELA_E2E_PASSWORD, and MELA_SUPABASE_ANON_KEY. Use a disposable test account/project with email confirmation disabled or a test confirmation mechanism; never use a production user account.'
  );
}
...
test.describe('authenticated registration', () => {
  test('registers a real user, establishes an authenticated session, creates the profile, and completes onboarding', async ({ page }) => {
    ...
    const email = E2E_EMAIL.replace(/^\s+|\s+$/g, '');
    ...
    // Registration must produce an authenticated session in the dedicated E2E environment.
    // If production email confirmation is enabled, use a disposable test project with a
    // deterministic confirmation mechanism rather than weakening the application or test.
    await expect
      .poll(async () => {
    ...
    const profileResponse = await page.request.get(
      `${SUPABASE_URL}/rest/v1/profiles?id=eq.${encodeURIComponent(user.id)}&select=id,full_name,preferred_language,role,account_status,profile_completion,onboarding_step,role_selected_at,email_verified,phone_verified&limit=1`,
  ...
  test('rejects a registration attempt with a password shorter than 12 characters', async ({ page }) => {
    requireRegistrationSecrets();
    ...
    await page.locator('#new').click();
    await expect(page.locator('#signup')).toBeVisible();
    await page.locator('#nm').fill('MELA E2E Invalid Password');
    await page.locator('#se').fill(`invalid-${Date.now()}@example.invalid`);
    await page.locator('#sp').fill('Short123!');
    await page.locator('#sl').selectOption('English');
    await page.locator('#create').click();
